...
import { createCharge, deleteCharge, deleteCharges } from "@/features/charges/actions";
import type { PaginatedResult, Charge, ChargeOverviewStats } from "@/features/charges/types";
...
import { AttachmentUploadField } from "../recettes/AttachmentUploadField";
import { EmbedPdfPreview } from "@/components/pdf/EmbedPdfPreview";
import { ConfirmActionDialog } from "@/components/shared/ConfirmActionDialog";
import { SelectionCheckbox } from "@/components/shared/SelectionCheckbox";

interface Props {
  result: PaginatedResult<Charge>;
  locations: Location[];
  categories: Category[];
  paymentMethods: PaymentMethod[];
  years: number[];
  overviewStats: ChargeOverviewStats;
}
...
export function ChargesClient({ result, locations, categories, paymentMethods, years, overviewStats }: Props) {
  ...
  const [freshChargeId, setFreshChargeId] = useState<number | null>(null);
  const [highlightChargeId, setHighlightChargeId] = useState<number | null>(null);
  ...
  const visibleChargeIds = useMemo(() => result.data.map((row) => row.id), [result.data]);
  const allVisibleChargesSelected =
    visibleChargeIds.length > 0 &&
    visibleChargeIds.every((id) => selectedChargeIds.includes(id));

  useEffect(() => {
    const visibleChargeIdSet = new Set(visibleChargeIds);
    setSelectedChargeIds((current) => current.filter((id) => visibleChargeIdSet.has(id)));
  }, [result.data, visibleChargeIds]);

  useEffect(() => {
    if (freshChargeId == null) return;
    const timer = window.setTimeout(() => setFreshChargeId(null), 3000);
  ...
  function resetFilters() {
    ...
    params.delete("page");
    router.push(`/charges${params.toString() ? `?${params.toString()}` : ""}`);
  }
  ...
  async function handleDelete() {
    ...
    try {
      const response = await deleteCharge(deleteTarget.id);
      ...
      toast.success("Charge supprimee avec succes.");
      router.refresh();
    } finally {
  ...
  async function handleBulkDelete() {
    if (selectedChargeIds.length === 0 || isBulkDeletePending) return;

    setIsBulkDeletePending(true);
    try {
      const response = await deleteCharges(selectedChargeIds);
      ...
      toast.success(`${response?.deletedCount ?? selectedChargeIds.length} charge(s) supprimee(s) avec succes.`);
      router.refresh();
    } finally {
  ...
  function toggleChargeSelection(id: number, checked: boolean) {
  ...
  function toggleAllVisibleCharges(checked: boolean) {
    setSelectedChargeIds(checked ? visibleChargeIds : []);
  }
  ...
  function handlePreview(charge: Charge) {
    if (!charge.attachment_path) return;
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white/85 p-4 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 md:p-4">
        ...
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Charges</h1>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
              Pilotage des depenses avec un suivi rapide par caisse, categorie et mode de paiement.
            </p>
          </div>
          ...
          {selectedChargeIds.length > 0 ? (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-rose-200/70 bg-rose-50/80 px-4 py-3 text-sm dark:border-rose-500/20 dark:bg-rose-500/10">
              <p className="font-medium text-rose-700 dark:text-rose-200">
                {selectedChargeIds.length} charge(s) selectionnee(s)
              </p>
              <div className="flex flex-wrap gap-2">
                <Button
                  type="button"
                ...
                <tr>
                  <th className="px-4 py-3 text-left font-semibold">
                    <SelectionCheckbox
                      ...
                      onCheckedChange={toggleAllVisibleCharges}
                      ariaLabel="Selectionner toutes les charges visibles"
                    />
                  </th>
              ...
              <tbody className="divide-y divide-slate-200/70 dark:divide-white/10">
                {result.data.length === 0 ? (
                  ...
                  result.data.map((charge) => {
                    const payment = paymentVisualMap.get(normalizePaymentMethod(charge.payment_method)) ?? FALLBACK_PAYMENT_VISUAL;
                    return (
                      <tr
                        key={charge.id}
                        data-search-highlight-id={charge.id}
                        className={cn(
                          "transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/40",
                          freshChargeId === charge.id && "new-row-flash",
                          highlightChargeId === charge.id && "search-row-highlight"
                        )}
                      >
                        <td className="px-4 py-3">
                          <SelectionCheckbox
                            checked={selectedChargeIds.includes(charge.id)}
                            onCheckedChange={(checked) => toggleChargeSelection(charge.id, checked)}
                            ariaLabel={`Selectionner la charge ${toSentenceCase(charge.description)}`}
                          />
                        </td>
                        <td className="px-4 py-3 text-slate-600 dark:text-slate-300">
                          {formatDateClear(charge.date)}
                        </td>
                        <td className="px-4 py-3 text-right font-semibold text-[var(--logo-secondary)]">
                          {formatAmountDh(charge.amount)}
                        </td>
                        <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">
                          {toSentenceCase(charge.description)}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{formatCaisseCode(charge.location_id)}</td>
                        <td className="px-4 py-3 text-center">
                          <span
                            title={charge.payment_method}
                        ...
                        <td className="px-4 py-3 text-right">
                          <ActionsMenu
                            ...
                            onPreview={() => handlePreview(charge)}
                          />
                        </td>
                      </tr>
                    );
                    ...
                    <div className="mt-3 flex items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full border border-slate-200/80 bg-white/75 px-2.5 py-1 text-xs font-medium text-slate-600 dark:border-white/10 dark:bg-slate-800/65 dark:text-slate-300">
                          {formatCaisseCode(charge.location_id)}
                        </span>
                        <span
                          title={charge.payment_method}
      ...
      <ConfirmActionDialog
        ...
        description={
          deleteTarget
            ? `La charge "${toSentenceCase(deleteTarget.description)}" pour ${formatAmountDh(deleteTarget.amount)} sera supprimee definitivement.`
            : "Cette charge sera supprimee definitivement."
        }
      ...
      <ConfirmActionDialog
        ...
        description={`Les ${selectedChargeIds.length} charge(s) selectionnee(s) seront supprimees definitivement.`}
...
function ChargeCreatePanel({
  ...
  useEffect(() => {
    ...
    if (state?.success) {
      ...
      if (typeof state.recordId === "number") {
        onCreated(state.recordId);
      }
      router.refresh();
    } else if (state?.error) {
...
function ActionsMenu({
  ...
  const hasAttachment = Boolean(charge.attachment_path);
  const downloadUrl = charge.attachment_id
    ? `/api/uploads/recettes/${charge.attachment_id}/preview?download=1`
    : (charge.attachment_path ?? "#");
  const fileName = getAttachmentStoredName(charge.attachment_path, charge.attachment_name);
  const downloadName = ensureFileExtension(fileName, charge.attachment_mime_type ?? "");
  return (
    <DropdownMenu>
...
function AttachmentPreviewDialog({
  ...
  const filePath = charge?.attachment_path ?? null;
  const previewPath = charge?.attachment_id ? `/api/uploads/recettes/${charge.attachment_id}/preview` : filePath;
  const downloadUrl = charge?.attachment_id
    ? `/api/uploads/recettes/${charge.attachment_id}/preview?download=1`
    : filePath;
  const mimeType = charge?.attachment_mime_type ?? "";
  const fileName = getAttachmentStoredName(filePath, charge?.attachment_name);
