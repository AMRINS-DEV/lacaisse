...
import { createCharge, updateCharge } from "@/features/charges/actions";
...
export function ChargeFormDialog({
  ...
  }, [state, charge, isEdit, onOpenChange, onSaved, router]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <form key={charge?.id ?? "new"} action={formAction} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                name="date"
                type="date"
                defaultValue={charge?.date ?? new Date().toISOString().split("T")[0]}
                required
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select name="category" defaultValue={charge?.category ?? ""} required>
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      name="payment_method"
                      value={pm.name}
                      defaultChecked={charge?.payment_method === pm.name}
                      required
              ...
              {locations.map((location) => (
                <label key={location.id} className="group cursor-pointer">
                  <input
                    ...
                    defaultChecked={charge?.location_id === location.id}
                    required
                    className="peer sr-only"
                  />
                  <span
          ...
          <AttachmentUploadField
            initialAttachmentId={charge?.attachment_id ?? null}
            initialAttachmentName={charge?.attachment_name ?? null}
            initialAttachmentPath={charge?.attachment_path ?? null}
            initialAttachmentMimeType={charge?.attachment_mime_type ?? null}
          />
